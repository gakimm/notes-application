<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('notes.show', $note->slug) }}" class="btn-secondary">
    ...
    <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
        <form action="{{ route('notes.update', $note->slug) }}" method="POST" x-data="noteEditor()" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="card">
                <div class="card-body">
                    <!-- Editor/Preview Area -->
                    <div class="min-h-96" x-init="content = '{{ old('content', $note->content) }}'">
                        <!-- Editor -->
                        <div x-show="!preview" class="space-y-2">
                            ...
                            <textarea name="content" 
                                    ...
                                    placeholder="Write your content in Markdown...">{{ old('content', $note->content) }}</textarea>
                            <p class="text-xs text-gray-500">You can use Markdown syntax for formatting.</p>
                        </div>

                        <!-- Preview -->
                        <div x-show="preview" class="space-y-2">
                            <label class="block text-sm font-medium text-gray-700">Preview</label>
                            <div class="border border-gray-300 rounded-md p-4 min-h-96 bg-gray-50 markdown-content" 
                                 x-html="content ? content.replace(/\n/g, '<br>') : '<p class=\'text-gray-400\'>Preview will appear here...</p>'">
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div x-data="{ 
                        visibility: '{{ old('visibility', $note->visibility) }}',
                        selectedUsers: {{ json_encode(old('shared_users', $note->sharedWithUsers ?? [])) }},
                        searchQuery: '',
                        searchResults: [],
                        isSearching: false,
                                ...
                                <div class="flex flex-col gap-4">
                                    <label class="flex items-center">
                                        <input type="radio" 
                                               ...
                                               value="private" 
                                               x-model="visibility"
                                               {{ old('visibility', $note->visibility) === 'private' ? 'checked' : '' }}
                                    ...
                                    <label class="flex items-center">
                                        <input type="radio" 
                                               ...
                                               value="shared" 
                                               x-model="visibility"
                                               {{ old('visibility', $note->visibility) === 'shared' ? 'checked' : '' }}
                                               class="form-radio text-primary-600">
                                        <span class="ml-2 text-sm text-gray-700">
                                    ...
                                    <label class="flex items-center">
                                        <input type="radio" 
                                               ...
                                               value="public" 
                                               x-model="visibility"
                                               {{ old('visibility', $note->visibility) === 'public' ? 'checked' : '' }}
                                               class="form-radio text-primary-600">
                                        <span class="ml-2 text-sm text-gray-700">
            ...
            <div class="card border-red-200">
                <div class="card-body">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-lg font-medium text-red-900">Delete Note</h3>
                            <p class="text-sm text-red-700">Once deleted, this note cannot be recovered.</p>
                        </div>
                        <button type="button" 
                                @click="if(confirm('Are you sure you want to delete this note? This action cannot be undone.')) { document.getElementById('delete-form').submit(); }" 
                                class="btn-danger">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Hidden Delete Form -->
        <form id="delete-form" action="{{ route('notes.destroy', $note->slug) }}" method="POST" class="hidden">
            @csrf
            @method('DELETE')
        </form>
    </div>
